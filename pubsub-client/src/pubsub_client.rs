...
fn connect_with_retry(
    ...
    loop {
        ...
        while n < headers_cp.len() {
            req = req.header(headers_cp[n][0].as_str(), headers_cp[n][1].as_str());
            n += 1;
        }
...
impl PubsubClient {
    ...
    pub fn vote_subscribe(url: &str, headers: Vec<[String;2]>) -> Result<VoteSubscription, PubsubClientError> {
        let url = Url::parse(url)?;
        let socket = connect_with_retry(url, headers)?;
        let (sender, receiver) = unbounded();
    ...
    pub fn root_subscribe(url: &str, headers: Vec<[String;2]>) -> Result<RootSubscription, PubsubClientError> {
        let url = Url::parse(url)?;
        let socket = connect_with_retry(url, headers)?;
        let (sender, receiver) = unbounded();
    ...
    pub fn slot_subscribe(url: &str, headers: Vec<[String;2]>,) -> Result<SlotsSubscription, PubsubClientError> {
        let url = Url::parse(url)?;
        let socket = connect_with_retry(url, headers)?;
